...
CURATE_SYSTEM = (
    ...
    "Every selected story MUST be a genuinely positive development: a good "
    "outcome achieved, concrete measurable progress, or a credible hopeful "
    "advance. REJECT — no matter how important or newsworthy — warnings, threats, "
    "risks, alarms, 'beware' pieces, disasters, crises, decline, backsliding, and "
    "any 'things are getting worse' story. A hard problem (a disease, the "
    "climate, a conflict) qualifies ONLY when the story's focus is a concrete "
    "solution, breakthrough, or advance against it, and that advance — not the "
    "threat — is the point. If in doubt, leave it out. "
...
WRITE_SYSTEM = (
    ...
    "considered whole rather than disconnected items. The tone is warm, "
    "grounded, and constructive — never alarmist. You may note a brief honest "
    "limitation where one genuinely exists, but you never turn a story into a "
    "warning, and you do not editorialise with cautions, threats, or 'beware' "
    "framing. Keep every segment tight and "
...
def story_prompt(story: dict) -> str:
    ...
    return (
        ...
        "clear why it matters, briskly, in a warm and constructive tone. You may "
        "add at most one short clause of background if it genuinely enriches the "
        "story, but do not add warnings, cautions, or a downbeat 'but beware' "
        "note. This is a "
