...
def _build_prompt(candidates: list[dict]) -> str:
    ...
    lines.append("")
    lines.append(CURATE_SCHEMA_INSTRUCTION)
    return "\n".join(lines)
...
def _validate(raw: str) -> list[dict]:
    ...
    if len(data) != EXPECTED_COUNT:
        raise CurationError(f"Expected exactly {EXPECTED_COUNT} stories, got {len(data)}")
...
def curate(candidates: list[dict]) -> tuple[list[dict], str]:
    """Select exactly 10 stories from ``candidates``.
    ...
    for attempt in (1, 2):
        log.info("Curation attempt %d (provider=%s model=%s)",
                 attempt, settings.curate_provider, settings.curate_model)
        last_raw = llm.call(
